 ...
 * TODO TCF 初始化加载IOC/DI依赖注入容器，基于反射给目标实例的目标属性注入属性值，基于反射调用目标实例的目标方法，实现依赖注入
 * @author 71485
 *
 */
public class IocHelper {
	...
	//TODO TCF 初始化启动IOC/DI依赖注入容器，基于反射给目标实例的目标属性注入属性值，基于反射调用目标实例的目标方法
			...
			if(beanMap!=null)
			{
				for(Map.Entry<Class<?>,Object> beanEntry:beanMap.entrySet())
				{
					...
					if(targetClass!=null && targetInstance!=null)
					{
						...
						if(fields!=null && fields.length>0)
						{
							for(Field field:fields)
							{
								...
								if(injectFieldFilter(field))
								{
									...
									{
										//TODO TCF 开始实现IOC/DI依赖注入
										if(fieldMap!=null)
										{
											for(Map.Entry<String,Map<String,Object>> fieldEntry:fieldMap.entrySet())
											{
												//TODO TCF 需要注入属性值的属性所在类simpleName
											    String classSimpleName=fieldEntry.getKey();
											    
											    //TODO TCF 当前加载的属性所在类simpleName必须和需要注入的属性所在类simpleName相同
											    if(StringUtils.isNotEmpty(classSimpleName) && classSimpleName.equals(targetClass.getSimpleName()))
											    {
											    	...
											    	if(paramMap!=null)
											    	{
											    		for(Map.Entry<String,Object> paramEntry:paramMap.entrySet())
											    		{
											    			...
											    			//TODO TCF 需要注入的属性名必须和当前加载的属性属性名相同才能注入
						...
						//TODO TCF 以下条件可以去掉，在这里的目的是只测试AOP动态代理织入增强处理方法执行效果
	...
	public static boolean invokeMethodFilter(Method method)
	{
		...
		//TODO TCF ...自定义过滤条件过滤需要调用的目标实例的目标方法
